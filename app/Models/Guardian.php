 ...
 * Guardian Model
 *
 * Formerly known as "Client" in the pet clinic system.
 * A guardian is a person who manages one or more patients (typically a parent managing children,
 * or a caregiver managing elderly/dependent patients).
 *
 * For adult self-managing patients, the patient may have their own account separate from a guardian.
 */
class Guardian extends Authenticatable implements HasName, HasAvatar, MustVerifyEmail, TaskRelated
{
    ...
    public function itemsToPay(): HasManyThrough|Guardian
    {
    ...
    {
        return $this->hasMany(Invoice::class, 'guardian_id');
    }
    ...
    {
        return $this->hasMany(Reservation::class, 'guardian_id');
    }

    public function lastReservation(): HasOne|Guardian
    {
        return $this->hasOne(Reservation::class, 'guardian_id')
            ->whereBeforeToday('date')
            ->latest();
    }

    public function nextReservation(): HasOne|Guardian
    {
        return $this->hasOne(Reservation::class, 'guardian_id')
